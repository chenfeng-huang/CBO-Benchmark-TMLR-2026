#!/usr/bin/env python3
"""
Generate Interventional Data for Chain Dataset (X, W, Z -> Y)

Interventions allowed on Z and W within [-1, 1] by default.
Saves:
- interventional_datasets/chain_interventional_BO.pkl
- interventional_datasets/chain_interventional_CBO.pkl
"""
...
def generate_cbo_interventional_data(graph, manipulative_variables, feature_params, target, task, n_samples=500):
    ...
    for intervention_set in cbo_vars:
        ...
        out.append([len(intervention_set), intervention_set, data_x, np.asarray(data_y)])
    return out


def save_interventional(bo_data, cbo_data):
    dataset_name = 'chain'
    interventional_dir = Path('interventional_datasets')
    interventional_dir.mkdir(exist_ok=True)
    bo_path = interventional_dir / f'{dataset_name}_interventional_BO.pkl'
    with open(bo_path, 'wb') as f:
        pickle.dump(bo_data, f)
    cbo_path = interventional_dir / f'{dataset_name}_interventional_CBO.pkl'
    with open(cbo_path, 'wb') as f:
        pickle.dump(cbo_data, f)
    print(f'Saved {bo_path} and {cbo_path}')
    return bo_path, cbo_path



def main():
    print('Generating interventional data for Chain dataset')
    np.random.seed(42)
    config, feature_params, df, sem_equations, model_results = load_chain_config()
    ...
    bo_data = generate_bo_interventional_data(graph, manipulative_variables, feature_params, target, task, n_samples=1000)
    cbo_data = generate_cbo_interventional_data(graph, manipulative_variables, feature_params, target, task, n_samples=500)
    save_interventional(bo_data, cbo_data)
    print('Chain interventional dataset generation complete.')
