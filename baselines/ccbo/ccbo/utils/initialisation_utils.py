...
def initialise_global_outcome_dict_new(
  ...
  targets = []
  if (isinstance(initial_optimal_target_values, list)
      and len(initial_optimal_target_values) > 0
      and initial_optimal_target_values[0] is not None):
    targets.append(float(initial_optimal_target_values[0]))
  else:
    # No interventional data was provided initially so use the blank value.
