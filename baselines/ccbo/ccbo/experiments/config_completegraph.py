...
"""Config for CompleteGraph experiment."""

import ml_collections


def get_config():
  """Return the default configuration for CompleteGraph experiment."""
  config = ml_collections.ConfigDict()

  # Name associated with this SCM
  config.example_name = 'completegraph'
  ...
  # Intervention variables based on CompleteGraph causal structure
  # F -> A -> E -> Y and B -> C -> D -> Y
  # We can intervene on F, A, B, C, D, E (all except target Y)
  ...
  # Intervention levels - using reasonable values based on variable ranges
  # F ~ N(0,1), A ~ F^2 + U1, B ~ U2, C ~ exp(-B), D ~ exp(-C)/10, E ~ cos(A) + C/10
  config.intervention_levels = (
      (0.,), (1.,), (0.,), (1.,), (0.1,), (0.,),  # Single variable interventions
      (0., 1.), (0., 1.), (1., 0.1), (1., 0.), (0.1, 0.),  # Two variable interventions
      (0., 0.), (1., 1.), (1., 0.)  # Additional combinations
  )  # Intervention values.

  config.constraints = ml_collections.ConfigDict()
  # Add constraints on some variables to make optimization more challenging
  config.constraints.variables = ('A', 'C', 'E')
  config.constraints.lambdas = (5., 2., 3.)  # Constraint values for A, C, E.

  # Exploration sets - same as intervention variables
