...
class CCBO(cbo.CBO):
  ...
  def __init__(
    ...
    # Be robust when no initial interventional samples are provided
    _safe_intervention_samples = (
        intervention_samples if isinstance(intervention_samples, dict) else {}
    )
    constraints_functions.initialise_constraints_interventional_objects(
        self.exploration_sets, _safe_intervention_samples,
  ...
  def _per_trial_computations(self, it: int, target: str,
    ...
    # Ensure y_new is a scalar value
    if isinstance(y_new, np.ndarray):
      y_new = float(y_new.item() if y_new.size == 1 else y_new.flatten()[0])
    ...
    if self.constraints_dict[best_es]["num"] == 0:
      ...
      for j in range(self.constraints_dict[best_es]["num"]):
        ...
        # Ensure y_new_var is a scalar value
        if isinstance(y_new_var, np.ndarray):
          y_new_var = float(y_new_var.item() if y_new_var.size == 1 else y_new_var.flatten()[0])
        y_new_var_noiseless = self.noiseless_target_functions[best_es](
            target=c_target,
            intervention_levels=np.squeeze(new_interventional_data_x))
        # Ensure y_new_var_noiseless is a scalar value
        if isinstance(y_new_var_noiseless, np.ndarray):
          y_new_var_noiseless = float(y_new_var_noiseless.item() if y_new_var_noiseless.size == 1 else y_new_var_noiseless.flatten()[0])
    ...
    if all(feasibility_list):
      ...
      y_new_scalar = float(np.asarray(y_new).item()) if isinstance(y_new, (np.ndarray, np.generic)) else float(y_new)
      current_best_scalar = float(np.asarray(current_best_global_target).item()) if isinstance(current_best_global_target, (np.ndarray, np.generic)) else float(current_best_global_target)
      
      best_value = utilities.EVAL_FN[self.task](y_new_scalar,
                                                current_best_scalar)
      ...
      new_best_solution = utilities.ARG_EVAL_FN[self.task](
          [y_new_scalar, current_best_scalar])
  ...
  def _get_current_feasible_global_target(self) -> Any:
    ...
    is_feasible = {}
    for es in self.exploration_sets:
      # Skip exploration sets with no interventional data yet
      if self.interventional_data_y[es] is None:
        continue
    ...
    # If no exploration sets have data yet, fall back to outcome_values
    if not out:
      return utilities.EVAL_FN[self.task](self.outcome_values)

    return utilities.EVAL_FN[self.task](out)
