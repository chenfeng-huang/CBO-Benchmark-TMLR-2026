...
from botorch.models.gp_regression import SingleTaskGP as FixedNoiseGP
...
class MultivariateNormalNetwork(Posterior):
    ...
    def base_sample_shape(self) -> torch.Size:
        r"""The base sample shape for sampling from the posterior."""
        # Return the shape needed for base samples (typically the number of test points)
        return torch.Size([self.X.shape[0]])

    @property
    def batch_range(self) -> tuple:
        r"""The batch range for the posterior."""
        # Return the range of batch dimensions (start, end)
        # For most cases, this is (0, -1) indicating all dimensions except the last are batch dims
        return (0, -1)

    def rsample_from_base_samples(self, sample_shape: torch.Size, base_samples: torch.Tensor) -> torch.Tensor:
        r"""Sample from the posterior using base samples."""
        # For now, we'll ignore base_samples and use the existing rsample method
