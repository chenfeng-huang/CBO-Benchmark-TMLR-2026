...
def create_base_dag_from_nodes(nodes, T=0, topology="dependent"):
    """
    Create a NetworkX DiGraph directly from node information.
    This replaces the need for pygraphviz conversion.
    """
    G = nx.MultiDiGraph()
    ...
    for t in range(T + 1):
        for node in nodes:
            node_name = f"{node}_{t}"
            G.add_node(node_name)
    
    # Add spatial edges based on topology
    for t in range(T + 1):
        if topology == "dependent":
            # Create chain: nodes[0] -> nodes[1] -> nodes[2] -> ...
            for i in range(len(nodes) - 1):
                G.add_edge(f"{nodes[i]}_{t}", f"{nodes[i+1]}_{t}")
        elif topology == "independent":
            # All nodes point to the last node (target)
            target_node = nodes[-1]
            for i in range(len(nodes) - 1):
                G.add_edge(f"{nodes[i]}_{t}", f"{target_node}_{t}")
    
    # Add temporal edges if T > 0
    if T > 0:
        for t in range(T):
            for node in nodes:
                G.add_edge(f"{node}_{t}", f"{node}_{t+1}")
...
def setup_stat_scm_ceo_extended_epidem(random_state, n_anchor, T: int = 0):
    ...
    base_dag = create_base_dag_from_nodes(["U", "T", "L", "R", "Y", "P", "M", "E", "Z", "X"], T=T)
    base_dag.T = T + 1
...
def setup_stat_scm_ceo_epidem(random_state, n_anchor, T: int = 0):
    ...
    base_dag = create_base_dag_from_nodes(["U", "T", "L", "R", "Y"], T=T)
    base_dag.T = T + 1
...
def setup_stat_scm_ceo_healthcare(random_state, n_anchor, T: int = 0):
    ...
    base_dag = create_base_dag_from_nodes(["A","B","R","S","C","Y"], T=T)
    base_dag.T = T + 1
...
def setup_stat_scm_ceo_toy(random_state, n_anchor, T: int = 0):
    ...
    base_dag = create_base_dag_from_nodes(["X", "Z", "Y"], T=T)
    # Set T attribute for all graphs (T+1 represents number of time steps, so T=0 means 1 time step)
    base_dag.T = T + 1
    # True is X -> Z -> Y
    true_dag = deepcopy(base_dag)
    graphs.append(true_dag)
    ...
    wrong_dag_2 = deepcopy(base_dag)
    wrong_dag_2.add_edge("X_0","Y_0")
    wrong_dag_2.T = T + 1
    graphs.append(wrong_dag_2)

    # Wrong graph: X <- Z -> Y
    wrong_dag_3 = create_empty_copy(base_dag)
    wrong_dag_3.add_edge("Z_0","Y_0")
    wrong_dag_3.add_edge("Z_0","X_0")
    #TODO
    wrong_dag_3.T = T + 1
    graphs.append(wrong_dag_3)
    ...
    wrong_dag_4.add_edge("Z_0","Y_0")
    wrong_dag_4.add_edge("X_0","Y_0")
    #TODO
    wrong_dag_4.T = T + 1
    graphs.append(wrong_dag_4)
    ...
    wrong_dag_5.add_edge("Z_0","X_0")
    wrong_dag_5.add_edge("X_0","Y_0")
    wrong_dag_5.add_edge("Z_0","Y_0")
    #TODO
    wrong_dag_5.T = T + 1
    ...
    wrong_dag_6.add_edge("Z_0","X_0")
    wrong_dag_6.add_edge("X_0","Y_0")
    #TODO
    wrong_dag_6.T = T + 1
