...
def fit_sem_emit_fncs(G: MultiDiGraph, D_obs: dict) -> dict:
    ...
    # Build strictly within time-slice emission functions
    for t in range(G.T):
        for child in G.nodes:
            # Use only nodes from current time-slice
            try:
                child_name, child_t_str = child.split("_")
                child_t = int(child_t_str)
            except Exception:
                continue
            if child_t != t:
                continue

            # Parents within the same time-slice
            parents_same_t = tuple(
                sorted(
                    filter(lambda x: x.endswith(str(t)), G.predecessors(child)),
                    key=sorted_nodes.get,
                )
            )

            if len(parents_same_t) > 0:
                # One or more within-slice parents: fit GP regression
                parent_vars = [pa.split("_")[0] for pa in parents_same_t]

                # Assemble X from parent variables at time t (fallback to last column if needed)
                xx_list = []
                for pv in parent_vars:
                    if t < D_obs[pv].shape[1]:
                        xx_list.append(D_obs[pv][:, t].reshape(-1, 1))
                    else:
                        xx_list.append(D_obs[pv][:, -1].reshape(-1, 1))
                xx = hstack(xx_list)

                # Assemble y from child at time t (fallback to last column if needed)
                if t < D_obs[child_name].shape[1]:
                    yy = D_obs[child_name][:, t].reshape(-1, 1)
                else:
                    yy = D_obs[child_name][:, -1].reshape(-1, 1)

                fncs[t][(parents_same_t, child)] = fit_gp(x=xx, y=yy)
            else:
                # Source node within this time-slice: fit marginal KDE
                if t < D_obs[child_name].shape[1]:
                    xx = D_obs[child_name][:, t].reshape(-1, 1)
                else:
                    xx = D_obs[child_name][:, -1].reshape(-1, 1)
