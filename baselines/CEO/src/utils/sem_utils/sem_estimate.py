...
def build_sem_hat(G: MultiDiGraph, emission_fncs: dict, transition_fncs: dict = None,) -> classmethod:
    ...
    class SEMHat:
        ...
        def _make_emit_fnc(moment: int) -> Callable:
            #  Within time-slice emission only, with defensive fallback to marginal when parents tuple is None
            def _emit(t, emit_input_vars, sample):
                try:
                    # If passed (None, child) treat as marginal for this child at time t
                    if (
                        emit_input_vars is not None
                        and isinstance(emit_input_vars, tuple)
                        and len(emit_input_vars) == 2
                        and emit_input_vars[0] is None
                    ):
                        child_key = (None, emit_input_vars[1])
                        return emission_fncs[t][child_key].predict()[moment]
                    return emission_fncs[t][emit_input_vars].predict(
                        select_sample(sample, emit_input_vars, t)
                    )[moment]
                except Exception:
                    # Final fallback: try marginal using provided child id
                    if isinstance(emit_input_vars, tuple) and len(emit_input_vars) == 2:
                        child_key = (None, emit_input_vars[1])
                        return emission_fncs[t][child_key].predict()[moment]
                    raise
            return _emit
