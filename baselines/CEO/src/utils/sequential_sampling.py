        ...
        if t == 0 or dynamic_sem is None:
            for var, function in static_sem.items():
                ...
                # If neither interventions nor initial values are provided; sample the model
                    ...
                    if nodeparents:
                        # Prefer calling emit/trans functions signature (t, inputs, sample)
                        try:
                            sample[var][t] = function(t, nodeparents, sample)
                        except TypeError:
                            # Fallback to marginal signature
                            sample[var][t] = function(t, (None, node))
                    else:
                        # Source node: prefer marginal signature (t, margin_id)
                        try:
                            sample[var][t] = function(t, (None, node))
                        except TypeError:
                            # If function expects sample as third arg (defensive)
                            sample[var][t] = function(t, (None, node), sample)
        else:
            ...
            for var, function in dynamic_sem.items():
                ...
                if interventions and interventions[var][t]:
                    ...
                    try:
                        ...
                        # If it's an emit-only function, it expects (t, emit_inputs, sample)
                        try:
                            sample[var][t] = function(t, node_parents(node, t), sample)
                        except TypeError:
                            # Final defensive fallback: marginal
                            sample[var][t] = function(t, (None, node))
                else:
