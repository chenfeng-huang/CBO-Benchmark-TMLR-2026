...
from src.utils.plotting import set_plotting
...
class BaseClassCeo:
        ...
        if temporal_index is not None:
            ...
            for k in self.all_sem_emit_fncs[graph_idx][temporal_index].keys():
                ...
                if k[-1] == node and res == k[0]:
                    return k
            # Fallbacks to ensure correct key shape for downstream functions
            if res:
                # Non-source node: return (parents_tuple, child_node)
                return (res, node)
            else:
                # Source node: return (None, child_node)
                return (None, node)
    ...
    def _per_trial_computations(self, t: int, it: int, target: str, assigned_blanket: dict, method: str = None):
        ...
        # Handle potential multi-dimensional arrays by taking the first element if needed
        target_value = all_variables_new[self.base_target_variable].squeeze()
        if hasattr(target_value, '__len__') and len(target_value) > 0:
            y_new = float(target_value[0])
        else:
            y_new = float(target_value)
            
        target_value_noiseless = all_variables_new_noiseless[self.base_target_variable].squeeze()
        if hasattr(target_value_noiseless, '__len__') and len(target_value_noiseless) > 0:
            y_new_noiseless = float(target_value_noiseless[0])
        else:
            y_new_noiseless = float(target_value_noiseless)
