...
sys.path.append("../src/")
sys.path.append("..")
import src  # ensure numpy shims are applied before any GPy import via src modules
...
# NumPy compatibility for older dependencies (e.g., GPy expects np.bool)
try:
    getattr(np, "bool")
...
parser.add_argument(
    "--seeds_replicate", type=int, nargs="+", default=[1, 2, 3], 
    help="seed for replicate: list of seeds for diff int. data (default: [1, 2, 3])"
)
parser.add_argument("--n_observational", type=int, default=100, 
                   help="number of observational samples (default: 100)")
parser.add_argument("--n_trials", type=int, default=10,
                   help="number of trials (default: 10)")
parser.add_argument("--n_anchor_points", type=int, default=5,
                   help="number of anchor points (default: 5)")
...
# ===== Save CSV for best-so-far per trial =====
import numpy as _np
import csv as _csv

_csv_path = os.path.join("results", "comparison_cd", f"best_so_far_{unique_id}.csv")
_labels_map = {
    'ceo': 'CEO',
    'cbo_wrong_avg': 'CBO with wrong G (avg)',
    'cbo_on_true_ei': 'CBO with true G',
}

# Align lengths and assemble rows: trial, ceo, cbo_on_true_ei, cbo_wrong_avg
_max_len = max(len(outs[m]) for m in methods_list)
_header = ["trial"] + [_labels_map[m] for m in methods_list]
_rows = []
for i in range(_max_len):
    _row = [i + 1]
    for m in methods_list:
        _series = outs[m]
        _val = float(_series[i]) if i < len(_series) else float('nan')
        _row.append(_val)
    _rows.append(_row)
...
print(f"Saved best-so-far CSV to: {_csv_path}")
