...
def update_sufficient_statistics_hat(
    ...
    def _cache_key(x: np.ndarray) -> tuple:
        # Fast immutable key avoiding expensive numpy pretty-print
        x_arr = np.asarray(x)
    ...
    def mean_function_internal(x_vals, mean_dict_store) -> np.ndarray:
        ...
        for x in x_vals:
            ...
            k = _cache_key(x)
            if k in mean_dict_store[temporal_index][exploration_set]:
                samples.append(mean_dict_store[temporal_index][exploration_set][k])
            else:
                ...
                mean_dict_store[temporal_index][exploration_set][k] = sample[target_variable][temporal_index]
    ...
    def variance_function_internal(x_vals, var_dict_store):
        ...
        for x in x_vals:
            ...
            k = _cache_key(x)
            if k in var_dict_store[temporal_index][exploration_set]:
                out.append(var_dict_store[temporal_index][exploration_set][k])
            else:
                # Otherwise compute it and store it
                for intervention_variable, xx in zip(exploration_set, x):
                    intervention_blanket[intervention_variable][temporal_index] = xx
                ...
                var_dict_store[temporal_index][exploration_set][k] = sample[target_variable][temporal_index]
