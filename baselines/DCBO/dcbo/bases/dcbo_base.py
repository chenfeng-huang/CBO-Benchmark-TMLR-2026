...
class BaseClassDCBO(Root):
    ...
    def _update_bo_model(
        ...
        if not self.bo_model[temporal_index][exploration_set]:
            ...
            # Constrain kernel parameters to safe bounds to avoid overflows
            try:
                model.kern.variance.constrain_bounded(1e-06, 5.0)
                model.kern.lengthscale.constrain_bounded(1e-03, 5.0)
            except Exception:
                pass
            ...
            # Limit optimizer iterations to avoid long stalls
            try:
            ...
            # Re-assert safe parameter bounds
            try:
                self.bo_model[temporal_index][exploration_set].model.kern.variance.constrain_bounded(1e-06, 5.0)
                self.bo_model[temporal_index][exploration_set].model.kern.lengthscale.constrain_bounded(1e-03, 5.0)
            except Exception:
                pass
            old_seed = np.random.get_state()

            np.random.seed(self.seed)
            # Disable emukit restarts and cap iterations
            try:
                self.bo_model[temporal_index][exploration_set].optimize(num_restarts=0, max_iters=100)
            except TypeError:
