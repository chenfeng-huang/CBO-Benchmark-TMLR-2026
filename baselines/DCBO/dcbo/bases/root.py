...
class Root:
    ...
    def _safe_optimization(self, temporal_index, exploration_set, bound_var=1e-06, bound_len=5.0) -> None:
        # Clamp kernel hyperparameters to numerically safe range
        var = self.bo_model[temporal_index][exploration_set].model.kern.variance
        ls = self.bo_model[temporal_index][exploration_set].model.kern.lengthscale
        if var[0] < bound_var:
            var[0] = bound_var
        if var[0] > 5.0:
            var[0] = 5.0
