...
class ProteinGraph(GraphStructure):
    ...
    def get_interventional_ranges(self):
        # PKC (C) and PKA (A) ranges are aligned with
        # CBO_Benchmark/configs/protein.yaml. HCBO's coverage algorithm
        # requires at least 3 intervenable variables, so we keep Mek (M)
        # intervenable using its observed-data range as a permissive bound.
        min_intervention_C = 0.5
        max_intervention_C = 106.5

        min_intervention_A = 1.45
        max_intervention_A = 4491.5

        min_intervention_M = 1.0
        max_intervention_M = 389.0
