...
from ...utils import *
import torch
# Test problems below are only used by the HDO baselines (REMBO/ALEBO/etc.)
# and not by the HCBO baseline. Their construction can break under newer
# botorch (stricter input bound checks), so we build them lazily.
def _safe(call):
    try:
        return call()
    except Exception as _e:  # noqa: BLE001
        return None
Ackly10Problem=_safe(lambda: get_Ackly_problem(10))
Ackly2Problem=_safe(lambda: get_Ackly_problem(2))
Hartmann6Problem=_safe(lambda: get_Hartmann6_problem(6))
Hartmann10Problem=_safe(lambda: get_Hartmann6_problem(10))
