...
def mcbo_trial(
    ...
    output_label = algo_profile.get('output_label') or algo_profile['algo']
    csv_filename = f"trial_results_{output_label}_{env_name}_{seed}.csv"
    csv_file = open(csv_filename, 'w', newline='')
    csv_writer = csv.DictWriter(csv_file, fieldnames=['trial_number', 'current_optimal'])
    ...
    csv_writer.writerow({'trial_number': 0, 'current_optimal': best_obs_val})
    csv_file.flush()
    ...
    for iteration in range(1, algo_profile["n_bo_iter"] + 1):
        ...
        csv_writer.writerow({'trial_number': iteration, 'current_optimal': best_obs_val})
        csv_file.flush()
