## Import basic packages
import numpy as np
import pandas as pd
from collections import OrderedDict
import scipy
import itertools
from numpy.random import randn
import copy
import seaborn as sns

import GPy
from GPy.kern.src.stationary import Stationary
from GPy.kern.src.psi_comp import PSICOMP_RBF, PSICOMP_RBF_GPU
from GPy.core import Param
from paramz.transformations import Logexp
from GPy.kern.src.grid_kerns import GridRBF


class CausalRBF(Stationary):
    """
    Radial Basis Function kernel, aka squared-exponential, exponentiated quadratic or Gaussian kernel:

    .. math::

       k(r) = \sigma^2 \exp \\bigg(- \\frac{1}{2} r^2 \\bigg)

    """
    _support_GPU = True
    def __init__(self, input_dim, variance_adjustment, variance=1., lengthscale=None, rescale_variance = 1., 
                    ARD=False, active_dims=None, name='rbf', useGPU=False, inv_l=False):
        super(CausalRBF, self).__init__(input_dim, variance, lengthscale, ARD, active_dims, name, useGPU=useGPU)
        if self.useGPU:
            self.psicomp = PSICOMP_RBF_GPU()
        else:
            self.psicomp = PSICOMP_RBF()
        self.use_invLengthscale = inv_l
        if inv_l:
            self.unlink_parameter(self.lengthscale)
            self.inv_l = Param('inv_lengthscale',1./self.lengthscale**2, Logexp())
            self.link_parameter(self.inv_l)
        self.variance_adjustment = variance_adjustment
        self.rescale_variance = Param('rescale_variance', rescale_variance, Logexp())
        #self.link_parameter(self.rescale_variance)



    def to_dict(self):
        """
        Convert the object into a json serializable dictionary.

        Note: It uses the private method _save_to_input_dict of the parent.

        :return dict: json serializable dictionary containing the needed information to instantiate the object
        """

        input_dict = super(CausalRBF, self)._save_to_input_dict()
        input_dict["class"] = "GPy.kern.RBF"
        input_dict["inv_l"] = self.use_invLengthscale
        if input_dict["inv_l"] == True:
            input_dict["lengthscale"] = np.sqrt(1 / float(self.inv_l))
        return input_dict


    def K(self, X, X2=None):
        """
        Kernel function applied on inputs X and X2.
        In the stationary case there is an inner function depending on the
        distances from X to X2, called r.

        K(X, X2) = K_of_r((X-X2)**2)
        """
        if X2 is None:
            X2 = X
        
        # Check dimensions and ensure X and X2 have adequate shapes
        if X.ndim == 1:
            X = X.reshape(-1, 1)
        if X2.ndim == 1:
            X2 = X2.reshape(-1, 1)
            
        # Ensure we have at least one dimension to operate on
        if X.shape[1] == 0:
            X = np.zeros((X.shape[0], 1))
        if X2.shape[1] == 0:
            X2 = np.zeros((X2.shape[0], 1))
        
        # Handle dimension mismatch - use only the dimensions both arrays have
        min_dims = min(X.shape[1], X2.shape[1], self.input_dim)
        
        # Extract only the dimensions we can use
        X_sub = X[:, :min_dims]
        X2_sub = X2[:, :min_dims]
        
        try:
            # Calculate scaled distance with the valid dimensions
            r = self._scaled_dist(X_sub, X2_sub)
            # Add epsilon for numerical stability
            r = np.clip(r, -1e10, 1e10)  # Clip to prevent overflow
            value = self.variance * np.exp(-0.5 * r**2)

            # Try variance adjustment calculation
            try:
                # Ensure X and X2 have the same number of dimensions for variance adjustment
                value_diagonal_X = self.variance_adjustment(X_sub) if min_dims == X.shape[1] else self.variance_adjustment(np.hstack([X_sub, np.zeros((X_sub.shape[0], self.input_dim - min_dims))]))
                value_diagonal_X2 = self.variance_adjustment(X2_sub) if min_dims == X2.shape[1] else self.variance_adjustment(np.hstack([X2_sub, np.zeros((X2_sub.shape[0], self.input_dim - min_dims))]))
                
                # Clip values to prevent numeric issues
                value_diagonal_X = np.clip(value_diagonal_X, 1e-10, 1e10)
                value_diagonal_X2 = np.clip(value_diagonal_X2, 1e-10, 1e10)
                
                # Reshape if needed
                if value_diagonal_X.ndim == 1:
                    value_diagonal_X = value_diagonal_X.reshape(-1, 1)
                if value_diagonal_X2.ndim == 1:
                    value_diagonal_X2 = value_diagonal_X2.reshape(-1, 1)
                
                # Calculate additional matrix
                additional_matrix = np.dot(np.sqrt(value_diagonal_X), np.sqrt(np.transpose(value_diagonal_X2)))
                # Ensure numeric stability in final value
                additional_matrix = np.clip(additional_matrix, -1e10, 1e10)
                return value + additional_matrix
            except Exception as e:
                # Print info about the error and fallback to basic kernel
                print(f"Warning in kernel variance adjustment: {e}")
                print(f"X shape: {X.shape}, X2 shape: {X2.shape}, min_dims: {min_dims}")
                return value
        except Exception as e:
            # Complete fallback for any kernel calculation error
            print(f"Error in kernel calculation: {e}")
            print(f"X shape: {X.shape}, X2 shape: {X2.shape}, input_dim: {self.input_dim}")
            # Return a simple fallback kernel
            return np.ones((X.shape[0], X2.shape[0])) * self.variance

    def Kdiag(self, X):
        """
        Compute the diagonal of the kernel matrix for input X.
        """
        # Check dimensions
        if X.ndim == 1:
            X = X.reshape(-1, 1)
        
        # Ensure we have at least one dimension to operate on
        if X.shape[1] == 0:
            X = np.zeros((X.shape[0], 1))
            
        # Initialize with base variance
        ret = np.full(X.shape[0], self.variance)
        
        try:
            # Handle dimension mismatch - use only the dimensions we have
            min_dims = min(X.shape[1], self.input_dim)
            X_sub = X[:, :min_dims]
            
            # Try to compute the variance adjustment
            try:
                # Ensure X has the expected number of dimensions
                if min_dims < self.input_dim:
                    # Pad with zeros if needed
                    X_padded = np.hstack([X_sub, np.zeros((X_sub.shape[0], self.input_dim - min_dims))])
                    value = self.variance_adjustment(X_padded)
                else:
                    value = self.variance_adjustment(X)
                
                # Add numeric stability
                value = np.clip(value, 1e-10, 1e10)
                
                # Handle different shapes of the output
                if np.isscalar(value):
                    diagonal_terms = np.full(X.shape[0], value)
                elif value.ndim == 1:
                    diagonal_terms = value
                else:
                    diagonal_terms = value.flatten()[:X.shape[0]]
                
                return ret + diagonal_terms
            except Exception as e:
                print(f"Warning in Kdiag variance adjustment: {e}")
                print(f"X shape: {X.shape}, input_dim: {self.input_dim}")
                return ret
        except Exception as e:
            print(f"Error in Kdiag calculation: {e}")
            return ret


    def K_of_r(self, r):
        return self.variance * np.exp(-0.5 * r**2)

    def dK_dr(self, r):
        return -r*self.K_of_r(r)

    def dK2_drdr(self, r):
        return (r**2-1)*self.K_of_r(r)

    def dK2_drdr_diag(self):
        return -self.variance # as the diagonal of r is always filled with zeros

    def __getstate__(self):
        dc = super(CausalRBF, self).__getstate__()
        if self.useGPU:
            dc['psicomp'] = PSICOMP_RBF()
            dc['useGPU'] = False
        return dc

    def __setstate__(self, state):
        self.use_invLengthscale = False
        return super(CausalRBF, self).__setstate__(state)

    def spectrum(self, omega):
        assert self.input_dim == 1 #TODO: higher dim spectra?
        return self.variance*np.sqrt(2*np.pi)*self.lengthscale*np.exp(-self.lengthscale*2*omega**2/2)

    def parameters_changed(self):
        if self.use_invLengthscale: self.lengthscale[:] = 1./np.sqrt(self.inv_l+1e-200)
        super(CausalRBF,self).parameters_changed()


    def get_one_dimensional_kernel(self, dim):
        """
        Specially intended for Grid regression.
        """
        oneDkernel = GridRBF(input_dim=1, variance=self.variance.copy(), originalDimensions=dim)
        return oneDkernel

    #---------------------------------------#
    #             PSI statistics            #
    #---------------------------------------#

    def psi0(self, Z, variational_posterior):
        return self.psicomp.psicomputations(self, Z, variational_posterior)[0]

    def psi1(self, Z, variational_posterior):
        return self.psicomp.psicomputations(self, Z, variational_posterior)[1]

    def psi2(self, Z, variational_posterior):
        return self.psicomp.psicomputations(self, Z, variational_posterior, return_psi2_n=False)[2]

    def psi2n(self, Z, variational_posterior):
        return self.psicomp.psicomputations(self, Z, variational_posterior, return_psi2_n=True)[2]

    def update_gradients_expectations(self, dL_dpsi0, dL_dpsi1, dL_dpsi2, Z, variational_posterior):
        dL_dvar, dL_dlengscale = self.psicomp.psiDerivativecomputations(self, dL_dpsi0, dL_dpsi1, dL_dpsi2, Z, variational_posterior)[:2]
        self.variance.gradient = dL_dvar
        self.lengthscale.gradient = dL_dlengscale
        if self.use_invLengthscale:
            self.inv_l.gradient = dL_dlengscale*(self.lengthscale**3/-2.)

    def gradients_Z_expectations(self, dL_dpsi0, dL_dpsi1, dL_dpsi2, Z, variational_posterior):
        return self.psicomp.psiDerivativecomputations(self, dL_dpsi0, dL_dpsi1, dL_dpsi2, Z, variational_posterior)[2]

    def gradients_qX_expectations(self, dL_dpsi0, dL_dpsi1, dL_dpsi2, Z, variational_posterior):
        return self.psicomp.psiDerivativecomputations(self, dL_dpsi0, dL_dpsi1, dL_dpsi2, Z, variational_posterior)[3:]

    def update_gradients_diag(self, dL_dKdiag, X):
        super(CausalRBF,self).update_gradients_diag(dL_dKdiag, X)
        if self.use_invLengthscale: self.inv_l.gradient =self.lengthscale.gradient*(self.lengthscale**3/-2.)

    def update_gradients_full(self, dL_dK, X, X2=None):
        super(CausalRBF,self).update_gradients_full(dL_dK, X, X2)
        if self.use_invLengthscale: self.inv_l.gradient =self.lengthscale.gradient*(self.lengthscale**3/-2.)

